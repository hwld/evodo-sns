...
- **D1 ORM**: Drizzle / Kysely / 直接 SQL のいずれか（11 章 拡張候補参照）
...
- `wrangler d1 migrations` を使用
- 適用は `wrangler d1 migrations apply <db> --remote` (本番) / `--local` (ローカル)
- 具体的な ORM / クエリビルダー（Drizzle / Kysely 等）の選定は拡張候補
